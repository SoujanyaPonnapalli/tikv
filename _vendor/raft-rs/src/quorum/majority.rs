...
impl Configuration {
    ...
    pub fn committed_index(&self, use_group_commit: bool, l: &impl AckedIndexer) -> (u64, bool) {
        ...
        // PATCHED FOR METRONOME: require ALL voters to have acked (rather
        // than a bare majority). This is variant (B) — guarantees that when
        // commit_index advances to i, every peer (including all persist-set
        // members) has durably stored entry i. Closes the safety race where
        // a leader could commit on K acks from stub-writing non-persisters.
        // See tikv/components/raftstore/src/store/metronome/scheme.rs.
        let quorum = matched.len();
